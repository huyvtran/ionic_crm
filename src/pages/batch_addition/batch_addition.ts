...
@Component({
  selector: "batch_addition",
  templateUrl: "batch_addition.html"
})
export class BatchAddition {
  constructor(
  ...
  backAndSave() {
    ...
    this.applyData.forEach(appItem => {
      ...
      if (!flag) {
        addPeople.push(appItem.customer);
      }
    });
    if (addPeople.length > 0) {
      ...
      this.mService
        .batch_addition(this.navParams.data[0].action, body)
        .then((res: any) => {
  ...
  getDataOfPage(apiName, recordType) {
    ...
    Promise.all([
      ...
      this.mService.getSearchData(this.initSearchBody)
    ]).then((res: any) => {
  ...
  changeAndPutData(data) {
    ...
    if (flag) {
      ...
      obj.attendee_department = this.getOptionsLabel(
        "department",
        data.department
      );
      ...
      obj.attendee_title = this.getOptionsLabel(
        "major_title",
        data.major_title
      );
